...
/**
 * Idempotent script wiring for the catalog tool: adds the
 * `ngx-iconify-stack:list-sets` script pointing at the read-only
 * `list-sets.mjs` tool generated by the skill schematic. The runner is
 * accepted for API symmetry with {@link wireSkillScript} but is not used —
 * the command runs `node` directly, so it is workspace-runner agnostic.
 * Returns 'added' | 'updated' | 'unchanged' so callers can log the right
 * message without duplicating the comparison logic.
 */
